...
static inline int
py_swig_to_libobs(const char *SWIG_type_str, PyObject *py_in, void *libobs_out)
{
...
static inline int libobs_to_py_swig(const char *SWIG_type_str,
                                    void *      libobs_in,
                                    int         ownership,
                                    PyObject ** py_out)
{
...
static bool is_python_on_path()
{
	...
	if (pypath == NULL) {
		blog(LOG_WARNING,
		     "%s:l%i \"Did not find 'PYTHONPATH' in environment "
		     "searching trying 'PYTHONHOME'\"",
		     __func__,
		     __LINE__);
	} else {
		return true;
	...
	if (pyhome == NULL) {
		blog(LOG_WARNING,
		     "%s:l%i \"Did not find 'PYTHONHOME' in environment "
		     "searching trying 'PATH'\"",
		     __func__,
		     __LINE__);
	} else {
		return true;
	...
	if (path) {
		...
		if (python) {
			...
			blog(LOG_WARNING,
			     "%s:l%i \"Did not find python in 'PATH' "
