...
// Check GCC
#if __GNUC__
#define PYTHON_SHARED_LIBRARY_NAME \
	"libpython" STR(PY_MAJOR_VERSION) "." STR(PY_MINOR_VERSION) "m.so"
...
bool obs_module_load()
{
	blog(LOG_INFO, "obs_module_load");
 	...
 	if (!is_python_on_path()) {
		blog(LOG_WARNING,
		     "%s:l%i \"Warning could not detect python environment "
		     "variables attempting to load shared library anyway\"",
		     __func__,
		     __LINE__);
	}
...
// Manually force python to be loaded
...
#endif
		blog(LOG_ERROR,
		     "%s:l%i \"Error Could not load python shared library %s "
		     "aborting!\"",
		     __func__,
		     __LINE__,
	...
	/* Setup logs to a safe place can be changed by user in
	 * OBSPythonManager.py register function*/
	...
	/* load the obspython library and extend with manually written
	 * functions/objects  */
	PyObject* py_libobs = PyImport_ImportModule("obspython");
	ret                 = pyHasError();
	if (ret) {
		blog(LOG_INFO,
		     "%s:l%i \"Error importing '%s/obspython.py' unloading "
		     "obs-python\"",
		     __func__,
		     __LINE__,
		     scripts_path);
	...
	if (ret) {
		blog(LOG_INFO,
		     "%s:l%i \"Error loading '%s/OBSPythonManager.py' "
		     "unloading obs-python\"",
		     __func__,
		     __LINE__,
		     scripts_path);
		goto out;
	...
	if (pModule != NULL) {
		...
		pFunc = PyObject_GetAttr(
		        pModule, PyUnicode_FromString("obs_module_load"));
		if (pFunc != NULL) {
			...
			if (ret) {
				blog(LOG_INFO,
				     "%s:l%i \"Error running 'register' "
				     "function in '%s/OBSPythonManager.py' "
				     "unloading obs-python\"",
				     __func__,
				     __LINE__,
				     scripts_path);
			...
			ret = pyHasError();
			blog(LOG_INFO,
			     "%s:l%i \"Could not find register function in "
			     "'%s/OBSPythonManager.py' unloading obs-python\"",
			     __func__,
			     __LINE__,
			     scripts_path);
		}
