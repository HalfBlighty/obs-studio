...
static PyObject *
py_source_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
...
static int py_source_init(py_source *self, PyObject *args, PyObject *kwds)
{
...
/*Method Table*/
static PyMethodDef py_source_methods[] = {{NULL} /* Sentinel */};
...
static PyMemberDef py_source_members[] = {
        ...
        {"get_width",
         T_OBJECT_EX,
         offsetof(py_source, get_width),
         0,
         "get_width"},
        {"get_height",
         T_OBJECT_EX,
         offsetof(py_source, get_height),
         0,
         "get_height"},
        {"get_defaults",
         T_OBJECT_EX,
         offsetof(py_source, get_defaults),
         0,
         "get_defaults"},
        {"get_properties",
         T_OBJECT_EX,
         offsetof(py_source, get_properties),
         0,
         "get_properties"},
        ...
        {"video_tick",
         T_OBJECT_EX,
         offsetof(py_source, video_tick),
         0,
         "video_tick"},
        {"video_render",
         T_OBJECT_EX,
         offsetof(py_source, video_render),
         0,
         "video_render"},
        {"filter_video",
         T_OBJECT_EX,
         offsetof(py_source, filter_video),
         0,
         "filter video"},
        {"filter_audio",
         T_OBJECT_EX,
         offsetof(py_source, filter_audio),
         0,
         "filter video"},
        {"enum_sources",
         T_OBJECT_EX,
         offsetof(py_source, enum_sources),
         0,
         "enum_sources"},
        ...
        {"mouse_click",
         T_OBJECT_EX,
         offsetof(py_source, mouse_click),
         0,
         "mouse_click"},
        {"mouse_move",
         T_OBJECT_EX,
         offsetof(py_source, mouse_move),
         0,
         "mouse_move"},
        {"mouse_wheel",
         T_OBJECT_EX,
         offsetof(py_source, mouse_wheel),
         0,
         "mouse_wheel"},
        {"focus", T_OBJECT_EX, offsetof(py_source, focus), 0, "focus"},
        {"key_click",
         T_OBJECT_EX,
         offsetof(py_source, key_click),
         0,
         "key_click"},
        {NULL} /* Sentinel */};
...
static void *py_source_create(obs_data_t *settings, obs_source_t *source)
{
	...
	SWIG_result = libobs_to_py_swig(
	        SWIG_str_obs_data_t, settings, 0, &py_swig_settings);

	if (!SWIG_IsOK(SWIG_result)) {
		blog(LOG_INFO,
		     "%s:l%i \"SWIG Failed to make required python object: "
		     "'%s' for '%s'\"",
		     __func__,
		     __LINE__,
		     SWIG_str_obs_data_t,
	...
	SWIG_result = libobs_to_py_swig(
	        SWIG_str_obs_source_t, source, 0, &py_swig_source);
...
static void py_source_destroy(void *data)
{
	...
	if (pyHasError()) {
		blog(LOG_INFO,
		     "Error destroying python object cleanly, this "
		     "could get messy");
	}
...
static obs_properties_t *py_source_properties(void *data)
{
	...
	if (!PyCallable_Check(py_src->get_properties)) {
		blog(LOG_INFO,
		     "None Callable get_properties: %s",
		     py_src->py_source_info->id);
		goto fail;
	}
	...
	SWIG_result = py_swig_to_libobs(
	        SWIG_str_obs_properties_t, py_result, &obs_properties);
	if (!SWIG_IsOK(SWIG_result)) {
		obs_properties = NULL;
	}
...
static void py_source_video_render(void *data, gs_effect_t *effect)
{
	...
	SWIG_result = libobs_to_py_swig(
	        SWIG_str_gs_effect_t, effect, 0, &py_swig_effect);
	if (!SWIG_IsOK(SWIG_result)) {
		goto fail;
	}
...
py_source_filter_video(void *data, struct obs_source_frame *frame)
{
	...
	if (!PyCallable_Check(py_src->filter_video)) {
		blog(LOG_INFO,
		     "None Callable filter_video: %s",
		     py_src->py_source_info->id);
		goto fail;
	}

	SWIG_result = libobs_to_py_swig(
	        SWIG_str_obs_source_frame, (void *)frame, 0, &py_frame);
	...
	SWIG_result =
	        py_swig_to_libobs(SWIG_str_obs_source_frame, py_ret, new_frame);

	if (!SWIG_IsOK(SWIG_result)) {
		blog(LOG_INFO,
		     "%s:l%i \"SWIG Failed to convert to libobs object: '%s' "
		     "for '%s'\"",
		     __func__,
		     __LINE__,
		     SWIG_str_obs_source_frame,
...
py_source_filter_audio(void *data, struct obs_audio_data *audio)
{
	...
	if (!PyCallable_Check(py_src->filter_audio)) {
		blog(LOG_INFO,
		     "None Callable filter_audio: %s",
		     py_src->py_source_info->id);
		goto fail;
	}

	SWIG_result = libobs_to_py_swig(
	        SWIG_str_obs_audio_data, (void *)audio, 0, &py_audio);
	...
	SWIG_result =
	        py_swig_to_libobs(SWIG_str_obs_audio_data, py_ret, new_audio);
                                  ...
                                  uint32_t                      click_count)
{
	...
	if (!PyCallable_Check(py_src->mouse_click)) {
		blog(LOG_INFO,
		     "None Callable mouuse_click: %s",
		     py_src->py_source_info->id);
		goto fail;
	}

	SWIG_result = libobs_to_py_swig(
	        SWIG_str_obs_mouse_event, (void *)event, 0, &py_swig_event);
                                  ...
                                  int                           y_delta)
{
	...
	argList = Py_BuildValue(
	        "(OOii)", py_data, py_swig_event, x_delta, y_delta);
...
py_source_key_click(void *data, const struct obs_key_event *event, bool key_up)
{
	...
	if (!PyCallable_Check(py_src->key_click)) {
		blog(LOG_INFO,
		     "None Callable key_click: %s",
		     py_src->py_source_info->id);
		goto fail;
	}

	SWIG_result = libobs_to_py_swig(
	        SWIG_str_obs_key_event, (void *)event, 0, &py_swig_event);
	...
	argList = Py_BuildValue(
	        "(OOO)", py_data, py_swig_event, key_up ? Py_True : Py_False);
...
static void py_to_obs_source_info(py_source *py_info)
{
	...
	/*The rest of the function pointers point to generic function that use
	 * void *data */
	...
	// register the type_data and its corresponding free
